...
def split_scale_make_sequences(
    df: pd.DataFrame,
    feat_cols: list,
    label: str,
    ...
    n = len(df)
    split_idx = get_train_size(n, train_proportion)

    df_train = df.iloc[:split_idx].copy()
    df_test = df.iloc[split_idx - lookback :].copy()

    scaler = StandardScaler()
    scaler.fit(df_train[feat_cols])

    df_train[feat_cols] = scaler.transform(df_train[feat_cols])
    df_test[feat_cols] = scaler.transform(df_test[feat_cols])

    X_train, y_train = make_lstm_sequences(
        df_train, feat_cols, label, lookback, use_delta
    )
    X_test, y_test = make_lstm_sequences(
        df_test, feat_cols, label, lookback, use_delta
    )
